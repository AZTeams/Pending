...
const char * const BITCOIN_CONF_FILENAME = "xgoldcoin.conf";
const char * const BITCOIN_PID_FILENAME = "xgoldcoin.pid";
...
#else
    const char* pszModule = "xgoldcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XGoldCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XGoldCoin";
#else
    // Unix
    return pathRet / ".xgoldcoin";
#endif
#endif
}
