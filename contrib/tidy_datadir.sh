...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete XGoldCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no XGoldCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old XGoldCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected XGoldCoin 0.7 datadir."
    ;;
  3)
    echo "Detected XGoldCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected XGoldCoin 0.8 datadir."
    ;;
esac
